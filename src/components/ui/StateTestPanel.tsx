 ...
 * Implementa M3 Segmented Buttons: https://m3.material.io/components/segmented-buttons/overview
 */
export function StateTestPanel() {
  ...
  const forceState = (id: string) => {
    ...
    if (id === 'normal') {
      ...
      if (devices[0]) selectDevice(devices[0].id)
      setTimeout(() => setAppState('loading-devices'), 10)
    } else if (id === 'error') {
      if (devices[0]) selectDevice(devices[0].id)
      setTimeout(() => setAppState('error-tracking', 'No pudimos conectar con el servidor de seguimiento. Verifica tu conexión e intenta de nuevo.'), 10)
    } else if (id === 'empty') {
  ...
  return (
    ...
    >
      <span style={{
        ...
        whiteSpace: 'nowrap', borderRight: '1px solid var(--color-outline-variant)',
        marginRight: 4,
      }}>DEMO</span>
          ...
          >
            {isActive && (
              <span className="material-symbols-outlined" aria-hidden="true"
                style={{fontSize: 12, color: 'var(--color-accent)', marginBottom: -2}}>
                check
              </span>
            )}
            <span className="material-symbols-outlined" aria-hidden="true"
